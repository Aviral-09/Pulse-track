...
const CohortsView = ({ projectId }) => {
    ...
    const fetchCohorts = async () => {
        ...
        setLoading(true);
        try {
            const res = await axios.get(`/api/v1/cohorts?project_id=${projectId}`);
            setCohorts(res.data);
        } catch (err) {
            console.error(err);
        } finally {
    ...
    const fetchEvents = async () => {
        if (!projectId) return;
        try {
            const res = await axios.get(`/api/v1/analytics/events?project_id=${projectId}`);
    ...
    const handlePreview = async () => {
        try {
            const res = await axios.post('/api/v1/cohorts/preview', {
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            await axios.post('/api/v1/cohorts', {
                project_id: projectId,
                ...newCohort
            });
            setIsCreating(false);
