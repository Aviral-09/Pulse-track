...
const Hyperspeed = ({
    ...
    useEffect(() => {
        ...
        `,
          ...
          vec3 getDistortion(float progress){
            ...
            return vec3( 
              cos(progress * PI * uFreq.x + uTime) * uAmp.x - cos(movementProgressFix * PI * uFreq.x + uTime) * uAmp.x,
              sin(progress * PI * uFreq.y + PI/2. + uTime) * uAmp.y - sin(movementProgressFix * PI * uFreq.y + PI/2. + uTime) * uAmp.x,
              0.
            );
          }
        `,
    ...
    return (
        <div id="lights" ref={hyperspeed} className={prefersReducedMotion ? 'bg-slate-950/20' : ''}>
            {prefersReducedMotion && (
                <div className="absolute inset-0 bg-gradient-to-br from-indigo-900/10 via-slate-900/10 to-emerald-900/10 animate-pulse-slow" />
            )}
        </div>
    );
};
